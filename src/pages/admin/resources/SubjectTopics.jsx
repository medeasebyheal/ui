...
import { Plus, Pencil, Trash2, FileQuestion, Video, FileText, Link as LinkIcon, Upload, Download } from 'lucide-react';
...
export default function SubjectTopics() {
  ...
  const loadSubjectResources = () => api.get(`/admin/subjects/${subjectId}/resources`).then(({ data }) => setSubjectResources(data || [])).catch(() => setSubjectResources([]));

  useEffect(() => {
    if (!yearId || !moduleId || !subjectId) return;
    Promise.all([loadMeta(), loadSubject(), loadTopics(), loadOneShotLectures(), loadSubjectResources()]).finally(() => setLoading(false));
  }, [yearId, moduleId, subjectId]);

  const handleResourceDelete = async (resourceId) => {
    try {
      await api.delete(`/admin/subjects/${subjectId}/resources/${resourceId}`);
  ...
  return (
    <>
      ...
      {/* Subject Resources (PDF + Link) */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mt-6">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-heading font-semibold text-gray-900">Subject Resources ({subjectResources.length})</h2>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setResourceForm({ type: 'pdf' })}
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-gray-300 text-gray-700 text-sm font-medium hover:border-primary/50 hover:text-primary transition-colors"
            >
        ...
        <ul className="divide-y divide-gray-100">
          {subjectResources.map((res) => (
            <li key={res._id} className="p-4 hover:bg-gray-50/50 flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {res.type === 'pdf' && (
                  <button
                    type="button"
                    onClick={(e) => handleResourceDownload(e, res)}
                    disabled={downloadingResourceId === res._id}
                ...
                <button
                  type="button"
                  onClick={() => setResourceForm({ resource: res })}
                ...
                <button
                  type="button"
                  onClick={() => setResourceDeleteConfirm(res)}
                  className="p-2 text-gray-500 hover:text-red-650 hover:text-red-600 rounded-lg"
                  title="Delete"
                >
        ...
        {subjectResources.length === 0 && (
          <div className="text-center py-12 bg-gray-50/50">
            ...
            <p className="text-sm text-gray-400 mt-1">Upload study guides or add resource links for this subject.</p>
          </div>
        )}
      </div>
      ...
      {resourceForm && (
        <SubjectResourceForm
          key={resourceForm.resource?._id ?? 'new'}
          resource={resourceForm.resource ?? null}
          type={resourceForm.type}
          subjectId={subjectId}
          onSave={loadSubjectResources}
      ...
      <ConfirmDialog
        ...
        message={resourceDeleteConfirm ? `Delete resource "${resourceDeleteConfirm.title}"?` : ''}
        confirmLabel="Delete"
...
function SubjectResourceForm({ resource, type: initialType, subjectId, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/admin/subjects/${subjectId}/resources/${resource._id}`, { title, url: url.trim() || undefined });
      } else if (type === 'pdf' && file) {
        const formData = new FormData();
        formData.append('type', 'pdf');
        formData.append('title', title);
        formData.append('file', file);
        await api.post(`/admin/subjects/${subjectId}/resources`, formData);
      } else if (type === 'link') {
        await api.post(`/admin/subjects/${subjectId}/resources`, { type: 'link', title, url: url.trim() });
      } else {
        setSaving(false);
        return;
      }
