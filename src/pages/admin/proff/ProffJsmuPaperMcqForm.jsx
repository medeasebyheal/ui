...
export default function ProffJsmuPaperMcqForm() {
  const { yearId, paperId, mcqId } = useParams();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const { data } = await api.get('/admin/proff/jsmu');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`${basePath}/mcqs/${mcqId}`, payload);
      } else {
        await api.post(`${basePath}/mcqs`, payload);
      }
      navigate(basePath);
    } catch (_) {
      setSaving(false);
  ...
  if (notFound || !year || !paper) {
    return (
      <div className="py-12">
        <p className="text-gray-500">Paper or MCQ not found.</p>
        <Link to="/admin/proff/jsmu" className="text-primary font-medium mt-2 inline-block">Back to JSMU</Link>
      </div>
    );
  }
      ...
      <div className="max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            ...
            <Link to={basePath} className="inline-flex items-center gap-2 px-5 py-2.5 border border-gray-300 rounded-xl font-medium text-gray-700 hover:bg-gray-50">
              Cancel
            </Link>
          </div>
        </form>
